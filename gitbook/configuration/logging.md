...
{% code title="edgeflow.hcl" lineNumbers="true" %}
```hcl
logging {
  level = "info"
...
For example, to set the logging level to `debug`, the format to `pretty`, the path to `/var/log/edgeflow`, and the rotation to `daily`, you can use the following configuration:
...
This will then create a log file named `edgeflow.log.<date>` in the `/var/log/edgeflow` directory and rotate it daily.
...
If a path is provided, the logs will be written to a file in the specified path. The file name will be prefixed or named `edgeflow.log.*`.
...
edgeflow --log.level debug

# Set the logging format to pretty
edgeflow --log.format pretty

# Set the logging path to /var/log/edgeflow
edgeflow --log.path /var/log/edgeflow

```

In this example, the logging level is set to `debug`, the format is set to `pretty`, the path is set to `/var/log/edgeflow`, and the rotation is set to `daily`.
