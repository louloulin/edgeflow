...
#
# EdgeFlow is a reverse proxy server that can be used to route incoming requests to different upstream servers based on the request's host, path, headers, and other attributes.
#
...
docker {
  ...
  # important: the lower the value, the more work EdgeFlow has to do.
  interval_secs = 15
...
routes = [
  {
    ...
    # Plugins can be used to extend the functionality of EdgeFlow.
