...
本指南将帮助您在5分钟内启动并运行EdgeFlow AI网关，体验其强大的AI流量管理功能。
...
```bash
git clone https://github.com/luizfonseca/edgeflow.git
cd edgeflow
cargo build --release
sudo cp target/release/edgeflow /usr/local/bin/
```

## 步骤2：创建配置文件

创建一个名为`edgeflow.hcl`的配置文件：

```hcl
// edgeflow.hcl - 基础AI网关配置
...
```bash
docker run -d \
  --name edgeflow-gateway \
  -p 8000:8000 \
  -v $(pwd)/edgeflow.hcl:/etc/edgeflow/edgeflow.hcl \
...
```
[INFO] EdgeFlow AI Gateway starting...
[INFO] Loading configuration from edgeflow.hcl
...
EdgeFlow提供了多个Web界面来监控和管理您的AI网关：
...
现在您已经成功设置了EdgeFlow AI网关，可以探索更多高级功能：
...
- **文档**：[https://docs.edgeflow.info](https://docs.edgeflow.info)
- **GitHub**：[https://github.com/luizfonseca/edgeflow](https://github.com/luizfonseca/edgeflow)
- **问题报告**：[GitHub Issues](https://github.com/luizfonseca/edgeflow/issues)
- **社区讨论**：[GitHub Discussions](https://github.com/luizfonseca/edgeflow/discussions)
...
3. **配置文件错误**
   ```bash
   # 验证HCL语法
   edgeflow -c edgeflow.hcl --validate
   ```
...
恭喜！您现在已经成功设置了一个功能完整的EdgeFlow AI网关。开始探索其强大的功能吧！
