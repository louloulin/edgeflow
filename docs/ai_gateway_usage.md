...
EdgeFlow AI网关是一个功能强大的反向代理解决方案，专为大型语言模型(LLM)流量管理而设计。通过EdgeFlow AI网关，您可以高效地路由、转换、安全检查并聚合与LLM提供商之间的请求。
...
EdgeFlow可以通过多种方式安装，包括直接二进制下载、Docker或从源代码构建。

### 二进制安装

```bash
curl -fsSL https://edgeflow.example.com/install.sh | sh
```
...
docker run -p 8000:8000 -v $(pwd)/config.hcl:/etc/edgeflow/config.hcl edgeflow/edgeflow
```
...
EdgeFlow使用HCL格式进行配置。以下是配置AI网关的关键部分。
...
使用EdgeFlow AI网关与直接调用LLM API类似，只是您将请求发送到EdgeFlow端点而不是直接发送到LLM提供商。
...
EdgeFlow提供详细的日志，可帮助诊断问题：

```bash
# 查看最近的错误日志
tail -f /var/log/edgeflow/error.log | grep "ai_gateway"

# 查看特定插件的日志
tail -f /var/log/edgeflow/access.log | grep "llm_router"
```
...
- 考虑将EdgeFlow部署在与LLM API相同的地区以减少延迟

## 扩展阅读

- [EdgeFlow架构概述](./architecture.md)
