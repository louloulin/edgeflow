...
EdgeFlow的异步API支持允许您处理长时间运行的AI请求，而无需保持连接等待响应。这对于批量处理、后台任务和高吞吐量场景特别有用。
...
```bash
# 查看异步处理日志
tail -f /var/log/edgeflow/async_api.log

# 过滤特定事件类型
grep "ai_request_completed" /var/log/edgeflow/async_api.log

# 查看错误日志
grep "ERROR" /var/log/edgeflow/async_api.log | tail -20
```
...
```bash
# 启用详细日志
export RUST_LOG=edgeflow::plugins::async_api=debug
