# BasicAuth Plugin for EdgeFlow

This plugin provides HTTP Basic Authentication for EdgeFlow, enabling users to verify the identity of API consumers through username/password credentials.
...
```rust
use crates::edgeflow::plugins::basic_auth::BasicAuthPlugin;
use crates::edgeflow::plugins::Plugin;
...
```rust
use crates::edgeflow::plugins::basic_auth::{BasicAuthPlugin, BasicAuthConfig};
use crates::edgeflow::plugins::Plugin;

let config = BasicAuthConfig {
    username: "admin".to_string(),
    password: "secure_password".to_string(),
    realm: "EdgeFlow API".to_string(),
    enabled: true,
};
...
| `realm` | String | The authentication realm name | "EdgeFlow Service" |
