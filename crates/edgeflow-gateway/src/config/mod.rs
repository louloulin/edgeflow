...
impl Default for Path {
    fn default() -> Self {
        Self {
            lets_encrypt: PathBuf::from("/etc/edgeflow/letsencrypt"),
        }
    }
}
...
pub struct RouteSslPath {
    /// Path to the certificate .key file (e.g. `/etc/edgeflow/certs/my-host.key`)
    pub key: PathBuf,

    /// Path to the certificate .pem file (e.g. `/etc/edgeflow/certs/my-host.pem`)
    pub pem: PathBuf,
}
...
///   production: true
/// paths:
///   config_file: "/etc/edgeflow/config.toml"
///   lets_encrypt: "/etc/edgeflow/letsencrypt"
/// routes:
///   - host: "example.com"
///     match_with:
///       path:
...
pub struct Config {
    ...
    #[serde(default)]
    #[clap(short, long, default_value = "edgeflow")]
    ...
    ///
    /// The configuration file should be named either `edgeflow.hcl` or `edgeflow.yaml`
    ///
...
impl Provider for Config {
    fn metadata(&self) -> figment::Metadata {
        figment::Metadata::named("edgeflow")
    }
...
/// E.g. `EDGEFLOW__LOGGING__LEVEL=DEBUG` will set the `level` key in the
/// `logging` key in the `edgeflow` key.
pub fn load(fallback: &str) -> Result<Config, figment::Error> {
    ...
    let config: Config = Figment::new()
        ...
        .merge(Yaml::file(format!("{path_with_fallback}/edgeflow.yml")))
        .merge(Yaml::file(format!("{path_with_fallback}/edgeflow.yaml")))
        .merge(Hcl::file(format!("{path_with_fallback}/edgeflow.hcl")))
        .merge(Env::prefixed("EDGEFLOW_").split("__"))
        .extract()?;
...
mod tests {
    ...
    fn test_load_config_from_yaml() {
        figment::Jail::expect_with(|jail| {
            ...
            jail.create_file(format!("{}/edgeflow.yaml", tmp_dir), helper_config_file())?;

            let config = load(&tmp_dir);
            let proxy_config = config.unwrap();
            assert_eq!(proxy_config.service_name, "edgeflow");
    ...
    fn test_load_config_from_yaml_and_env_vars() {
        figment::Jail::expect_with(|jail| {
            jail.create_file(
                format!("{}/edgeflow.yaml", jail.directory().to_str().unwrap()),
                helper_config_file(),
            )?;
            jail.set_env("EDGEFLOW_SERVICE_NAME", "new_name");
            jail.set_env("EDGEFLOW_LOGGING__ENABLED", "false");
            jail.set_env("EDGEFLOW_LOGGING__LEVEL", "warn");
            jail.set_env("EDGEFLOW_DOCKER__ENABLED", "true");
            jail.set_env("EDGEFLOW_DOCKER__INTERVAL_SECS", "30");
            jail.set_env("EDGEFLOW_DOCKER__ENDPOINT", "http://localhost:2375");
            jail.set_env("EDGEFLOW_LETS_ENCRYPT__STAGING", "false");
            jail.set_env("EDGEFLOW_LETS_ENCRYPT__EMAIL", "my-real-email@domain.com");
            jail.set_env("EDGEFLOW_LETS_ENCRYPT__RENEW_INTERVAL_SECS", "60");
            jail.set_env(
                "EDGEFLOW_ROUTES",
                r#"[{
    ...
    fn test_load_config_with_defaults_and_yaml() {
        figment::Jail::expect_with(|jail| {
            ...
            jail.create_file(
                ...
                routes:
                  - host: "example.com"
                    ...
                    ssl:
                      path:
                        key: "/etc/edgeflow/certs/my-host.key"
                        pem: "/etc/edgeflow/certs/my-host.pem"
                "#,
            )?;
            ...
            assert_eq!(paths.lets_encrypt.as_os_str(), "/etc/edgeflow/letsencrypt");
            ...
            assert_eq!(path.key.as_os_str(), "/etc/edgeflow/certs/my-host.key");
            assert_eq!(path.pem.as_os_str(), "/etc/edgeflow/certs/my-host.pem");
