...
pub struct EdgeFlowDockerRoute {
...
impl LabelService {
    ...
    ) -> HashMap<String, EdgeFlowDockerRoute>
    where
        T: Into<String> + Hash + serde::ser::Serialize + Eq,
    {
        let mut host_map = HashMap::<String, EdgeFlowDockerRoute>::new();
        let services = self
            .inner
        ...
        for service in services {
            ...
            for (k, v) in service_labels {
                if k.starts_with("edgeflow.") {
                    // direct values
                    // TODO refactor to be reused for both services and containers
                    match k.as_str() {
                        "edgeflow.enabled" => proxy_enabled = v == "true",
                        "edgeflow.host" => proxy_host = v,
                        "edgeflow.port" => proxy_port = v,
                        k if k.starts_with("edgeflow.match_with.path.pattern.") => {
                            match_with_path_patterns.push(v.clone());
                        }
                        "edgeflow.headers.add" => {
                            let deser: Vec<RouteHeaderAdd> =
                                serde_json::from_str(v).unwrap_or(vec![]);

                            route_header_add = Some(deser);
                        }
                        "edgeflow.headers.remove" => {
                            let deser: Vec<RouteHeaderRemove> =
                                serde_json::from_str(v).unwrap_or(vec![]);

                            route_header_remove = Some(deser);
                        }
                        "edgeflow.ssl_certificate.self_signed_on_failure" => {
                            ssl_certificate_self_signed_on_failure = v == "true";
                        }
                        "edgeflow.plugins.oauth2.provider" => oauth2_provider = Some(v.clone()),
                        "edgeflow.plugins.oauth2.client_id" => oauth2_client_id = Some(v.clone()),
                        "edgeflow.plugins.oauth2.client_secret" => {
                            oauth2_client_secret = Some(v.clone());
                        }
                        "edgeflow.plugins.oauth2.jwt_secret" => oauth2_jwt_secret = Some(v.clone()),
                        "edgeflow.plugins.oauth2.validations" => {
                            oauth2_validations =
                                Some(serde_json::from_str(v).unwrap_or_else(|_| json!([])));
                        }
                        "edgeflow.plugins.request_id.enabled" => {
                            docker_request_id = v == "true";
                        }
                        "edgeflow.plugins.basic_auth.user" => basic_auth_user = Some(v.clone()),
                        "edgeflow.plugins.basic_auth.password" => {
            ...
            if proxy_host.is_empty() || proxy_port.is_empty() {
                info!(
                    "Service {service_name:?} does not have the label
                    edgeflow.host set to a valid host or edgeflow.port set to a valid port"
                );
                continue;
            }
            ...
            if !host_map.contains_key(proxy_host) {
                let mut routed = EdgeFlowDockerRoute::default();
                routed
                    .upstreams
        ...
        for container in containers {
            ...
            if proxy_port.is_empty() || proxy_host.is_empty() {
                info!(
                  ...
                  `edgeflow.port` label or a `edgeflow.host` label"
                );
                continue;
            }
            ...
            if !host_map.contains_key(proxy_host) {
                let mut routed = EdgeFlowDockerRoute::new(vec![], match_with_path_patterns);
                routed.host_header_add = route_header_add;
                routed.host_header_remove = route_header_remove;
                routed.ssl_certificate_self_signed_on_failure =
                    ssl_certificate_self_signed_on_failure;
            ...
            for network in networks.values() {
                ...
                // skip values from networks that EdgeFlow does not have access to
    ...
    fn send_route_message(&self, hosts: HashMap<String, EdgeFlowDockerRoute>) {
    ...
    // So that EdgeFlow can route the appropriate traffic
    async fn get_routes_from_docker(&self) -> HashMap<String, EdgeFlowDockerRoute> {
        let mut filters = HashMap::new();
        filters.insert(
            "label",
            vec!["edgeflow.enabled=true", "edgeflow.host", "edgeflow.port"],
        );
